...
import { validarCNPJ, formatarCNPJ, formatarTelefone } from "../../utils/validators";
...
export function RegisterScreen({ onSwitchToLogin }: RegisterScreenProps) {
  ...
  // ==== Tailwind sem classe dinâmica ====
  const isCliente = selectedRole === "usuario";
  const gradientClass = isCliente
    ? "bg-gradient-to-br from-blue-500 to-blue-700"
    : "bg-gradient-to-br from-green-500 to-green-700";

  const iconBgClass = isCliente ? "bg-blue-500" : "bg-green-500";

  const buttonClass = isCliente
    ? "w-full bg-blue-500 hover:bg-blue-600"
    : "w-full bg-green-500 hover:bg-green-600";

  const roleLabel = isCliente ? "Cliente" : "Profissional";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "telefone") {
      const tel = formatarTelefone(value);
      setFormData((prev) => ({ ...prev, telefone: tel }));
      return;
    }
    ...
    if (name === "cnpj") {
      ...
      setCnpjValido(cnpjLimpo.length === 14 ? validarCNPJ(cnpjFormatado) : null);
      return;
    }
  ...
  const handleSelectRole = (role: UserRole) => {
    ...
    if (role === "usuario") {
      setFormData((prev) => ({ ...prev, categoria: "", cnpj: "" }));
    }
  };
  ...
  const formatarTelefone = (v: string) => {
    const d = v.replace(/\D/g, "").slice(0, 11); // 11 dígitos (DDD + 9 dígitos)
    ...
    try {
      if (!selectedRole) throw new Error("Selecione o tipo de cadastro");
      ...
      await register(formData, selectedRole, foto);
    } catch (err: any) {
  ...
  // ===== Etapa 2: Formulário =====
  return (
    <div className={`min-h-screen ${gradientClass} flex items-center justify-center p-4`}>
